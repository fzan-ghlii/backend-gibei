...
export class RegistrationsService {
  ...
  async registerEvent(userId: number, eventId: number) {
    ...
    if (existing) throw new ConflictException('Anda sudah terdaftar di event ini');
  ...
  async registerSpm(userId: number, spmId: number, dto: RegisterSpmDto) {
    ...
    if (existing) throw new ConflictException('Anda sudah terdaftar di jadwal SPM ini');
